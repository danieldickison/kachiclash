...
const pickForm = document.getElementById(
  "banzuke-select-rikishi-form",
) as HTMLFormElement;
const heyaSelect = document.getElementById(
  "heya-select",
...
for (const el of document.querySelectorAll(".select-radio")) {
  ...
  radio.addEventListener("change", (_event) => {
    for (const otherRadio of document.getElementsByName(radio.name)) {
      const label = pickForm.querySelector(
        `label.click-target[for="${otherRadio.id}"]`,
      ) as HTMLElement;
