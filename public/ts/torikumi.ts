...
const torikumiForm = document.getElementById(
  "torikumi-form",
) as HTMLFormElement;
...
function torikumiFormInput(): void {
  ...
  const tbody = torikumiForm.querySelector(
    ".parsed-torikumi tbody",
...
const TORIKUMI_REGEX =
  /^ *[a-z]{1,2}\d{1,3}[ew] +([a-z]+) +\(\d+(?:-\d+){1,2}\) +[a-z]+ *[a-z]{1,2}\d{1,3}[ew] +([a-z]+) +\(\d+(?:-\d+){1,2}\) *$/gim;
  ...
  fetch(postURL, {
    ...
    .then(async (response) => {
      if (response.ok) {
        ...
        return await response.text().then((msg) => {
          throw new Error(msg);
        });
      }
    })
    .catch((err: Error) => {
      alert("error updating torikumi: " + err.toString());
    });
});
