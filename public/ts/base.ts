...
function initBashoCountDown (): void {
  for (const timeSpan of document.querySelectorAll('.js-basho-count-down')) {
    if (!(timeSpan instanceof HTMLElement) || timeSpan.dataset.startDate === undefined) {
...
async function initPushPromo (): Promise<void> {
  ...
  if (promo === null || localStorage.getItem('push-promo-dismissed') === '1' || await pushPermissionState() !== 'prompt') {
    return
  }
